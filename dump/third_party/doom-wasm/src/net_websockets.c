...
static void WebSocketsSendTracHello(void)
{
    int addr_arg = M_CheckParmWithArgs("-tracaddr", 1);
    if (addr_arg <= 0) return;
    const char *addr = myargv[addr_arg + 1];
    if (addr == NULL || addr[0] == '\0') return;
    const char *nick = NULL;
    int nick_arg = M_CheckParmWithArgs("-tracnick", 1);
    if (nick_arg > 0) {
        nick = myargv[nick_arg + 1];
        if (nick != NULL && nick[0] == '\0') nick = NULL;
    }
    char msg[256];
    if (nick) {
        M_snprintf(msg, sizeof(msg), "{\"t\":\"mhello\",\"uid\":%u,\"address\":\"%s\",\"nick\":\"%s\"}", instanceUID, addr, nick);
    } else {
        M_snprintf(msg, sizeof(msg), "{\"t\":\"mhello\",\"uid\":%u,\"address\":\"%s\"}", instanceUID, addr);
    }
    emscripten_websocket_send_utf8_text(websocket, msg);
}
...
void NET_Websockets_SendKill(unsigned int killer_uid, unsigned int victim_uid)
{
    if (killer_uid == 0 || victim_uid == 0) return;
    if (!inittedWebSockets) {
        if (InitWebSockets() == false) return;
    }
    char msg[128];
    M_snprintf(msg, sizeof(msg), "{\"t\":\"klog\",\"killerUid\":%u,\"victimUid\":%u}", killer_uid, victim_uid);
    emscripten_websocket_send_utf8_text(websocket, msg);
}
