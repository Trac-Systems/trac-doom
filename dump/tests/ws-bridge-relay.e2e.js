...
function makePeer ({ isIndexer = false, channel = 'e2e' } = {}) {
  return {
    swarm: new FakeSwarm(),
    wallet: { publicKey: hex(32) },
    base: { isIndexer },
    protocol_instance: { api: { getNick: async () => null, recordKillStrict: async () => {} } },
    options: { channel }
  }
}
...
async function main () {
  ...
  const indexer = makePeer({ isIndexer: true, channel: 'e2e' })
  const host = makePeer({ channel: 'e2e' })
  const joinerA = makePeer({ channel: 'e2e' })
  const joinerB = makePeer({ channel: 'e2e' })
