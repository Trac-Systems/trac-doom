...
export function attachDoomWSBridge (peer, { port = 7788, host = '127.0.0.1', disableWS = false } = {}) {
  ...
  function queueGameFrame (gid, buf) {
    const now = Date.now()
  ...
  function flushGameQueue () {
    ...
    for (const entry of queue) {
      ...
      let localSent = 0
      try {
        const view = new DataView(entry.buf.buffer, entry.buf.byteOffset, entry.buf.byteLength)
  ...
  function destroyMatchSwarm (reason) {
    ...
    state.gameQueue = []
    try { globalThis.__lastServerPresence = null } catch {}
  ...
  function ensureGameChannel (connection, gid) {
    ...
    const message = ch.addMessage({ encoding: c.raw, onmessage (buf) {
      try {
        ...
        const delivered = deliverToDoomClients(to, from, payload)
        try { if (delivered === 0) log('debug', `[ws-bridge] p2p->ws drop gid=${gid} to=${to} from=${from} clients=${state.clients.size}`) } catch {}
        ...
        try {
          ...
          if (to !== 0) {
            ...
            if (dest) {
              ensureGameChannel(dest, gid).message.send(buf)
              fwd = 1
            } else {
              for (const conn of gameConnections()) {
                if (conn === connection) continue
                ensureGameChannel(conn, gid).message.send(buf)
                fwd++
              }
            }
          } else {
          ...
          if (fwd) log('debug', `[ws-bridge] p2p->p2p fwd gid=${gid} to=${to} from=${from} hops=${fwd}`)
        } catch {}
      } catch {}
    }})
    const obj = { message }
  ...
  function ensureDefaultChannel (connection) {
    ...
    const message = ch.addMessage({ encoding: c.raw, onmessage (buf) {
      try {
        ...
        rememberRoute(from, connection)
        const delivered = deliverToDoomClients(to, from, payload)
        try { if (delivered === 0) log('debug', `[ws-bridge] p2p->ws drop(default) to=${to} from=${from} clients=${state.clients.size}`) } catch {}
        // Forward onward to other swarm connections for bootstrap
        try {
          let fwd = 0
          if (to !== 0) {
            const dest = selectRoute(to, connection)
            if (dest) {
              ensureDefaultChannel(dest).message.send(buf)
              fwd = 1
            } else {
              for (const conn of gameConnections()) { if (conn === connection) continue; ensureDefaultChannel(conn).message.send(buf); fwd++ }
            }
          } else {
            for (const conn of gameConnections()) { if (conn === connection) continue; ensureDefaultChannel(conn).message.send(buf); fwd++ }
          }
          if (fwd) log('debug', `[ws-bridge] p2p->p2p fwd(default) to=${to} from=${from} hops=${fwd}`)
        } catch {}
      } catch {}
    }})
    const obj = { message }
  ...
  function sendServerPresence (connection) {
    try {
      const pres = globalThis.__lastServerPresence
        ...
        if (u8.byteLength >= 8) {
          ...
          if (gid) {
            ...
            if (to !== 0) {
              const dest = selectRoute(to, null)
              if (dest) {
                ensureGameChannel(dest, gid).message.send(u8)
                sent = 1
              } else {
                for (const conn of conns) { ensureGameChannel(conn, gid).message.send(u8); sent++ }
              }
            } else {
              for (const conn of conns) { ensureGameChannel(conn, gid).message.send(u8); sent++ }
            }
            ...
            if (!state.matchSwarm) {
              for (const conn of conns) { ensureDefaultChannel(conn).message.send(u8) }
            }
            try { log('debug', `[ws-bridge] ws->p2p gid=${gid} to=${to} from=${from} conns=${sent}`) } catch {}
            if (to === 0 && from === 1) try { globalThis.__lastServerPresence = u8.slice() } catch {}
          } else {
            // bootstrap path before gid is known
            let conns = 0
            for (const conn of gameConnections()) { ensureDefaultChannel(conn).message.send(u8); conns++ }
            try { log('debug', `[ws-bridge] ws->p2p bootstrap to=${to} from=${from} conns=${conns}`) } catch {}
            if (to === 0 && from === 1) try { globalThis.__lastServerPresence = u8.slice() } catch {}
          }
        }
      } catch {}
  ...
  function onConnection (ws, kind) {
    ...
    if (kind === 'doom' && globalThis.__lastServerPresence instanceof Uint8Array) {
      try {
        const pres = globalThis.__lastServerPresence
