...
unsigned int NET_SV_GetPlayerUID(int player)
{
    if (player < 0 || player >= NET_MAXPLAYERS) return 0;
    if (sv_players[player] == NULL || sv_players[player]->addr == NULL) return 0;
    if (sv_players[player]->addr->handle != NULL) {
        return (*(uint32_t *)(sv_players[player]->addr->handle));
    }
    // loopback/local server client
    return 1;
}

void NET_SV_RecordKill(int killer_player, int victim_player)
{
    if (M_CheckParm("-server") <= 0 && M_CheckParm("-privateserver") <= 0) return;
    unsigned int killer_uid = NET_SV_GetPlayerUID(killer_player);
    unsigned int victim_uid = NET_SV_GetPlayerUID(victim_player);
    if (killer_uid == 0 || victim_uid == 0) return;
    NET_Websockets_SendKill(killer_uid, victim_uid);
}
